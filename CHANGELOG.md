...
### Editable action group reassignment

- Root cause: `ActionEditor.tsx` rendered the `グループ` field as a read-only label sourced from the group the action was originally created under (`groupName` prop), with `nextAction.group_id` always copied straight from `action?.group_id` on save. There was no UI path to change an existing action's group; moving one required deleting and recreating it, which risked losing trigger/action configuration and produced a new action identity.
- Fix: `ActionEditor.tsx` now takes the full `groups: ActionGroup[]` list (instead of a single `groupName` string) and renders `グループ` as a `<select>`, matching the existing トリガー種類/Trigger Slot controls. A `groupId` field is tracked in component state, initialized via `resolveInitialGroupId` to the action's current `group_id` if it matches a known group, or the first configured group otherwise (covering a stored `group_id` that no longer exists). The autosave effect for existing actions now includes `groupId` in its dependency list, so picking a different group and waiting for the debounced autosave (or clicking 保存 for a new action) saves the action with the new `group_id` and nothing else changed. `ActionsTab.tsx` passes `groups={groups}` instead of `groupName={selectedGroupName}`.
- No backend change was required: `Config::normalized`/`normalize_groups_and_actions` in `src-tauri/src/config.rs` already resolves any `group_id` against the known group set on every save/load, falling back an unknown or deleted group reference to the default `group-uncategorized` group without dropping the action. Because the store's `updateAction` replaces the action in place by array position rather than moving elements between per-group arrays, changing only `group_id` is sufficient to move the action to the destination group's section in `ActionList.tsx` (which buckets by `group_id` at render time) with no risk of duplication or ID churn.
- Added regression tests in `src-tauri/src/config.rs`: editing an action's `group_id` and normalizing moves it into the destination group while leaving unrelated actions' groups untouched; the moved action's name/trigger_type/trigger_slot/gesture/action_type/keystroke/modifiers are unchanged and only `group_id` differs; moving to each configured group resolves to that same group without inventing duplicates; an unknown/deleted `group_id` falls back to `group-uncategorized` without dropping the action; and a save/reload round trip preserves the reassignment with the action appearing exactly once and no unrelated action's group changing.

Source basis:

- Current `src/components/actions/ActionEditor.tsx`, `src/components/actions/ActionsTab.tsx`, `src-tauri/src/config.rs`.
